            ...
            <div className="space-y-6 pt-12">
              {NETWORK_NODES.slice(0, 2).map((node, idx) => {
                ...
                return (
                  ...
                  >
                    <Icon className={cn('mb-4', isDark ? 'text-brand-accent' : 'text-brand-primary')} />
                    <h4
                      className={cn(
            ...
            <div className="space-y-6">
              {NETWORK_NODES.slice(2, 4).map((node, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="bg-brand-surface p-8 rounded-[2rem] shadow-xl shadow-brand-primary/5 border border-brand-border"
                  >
                    <Icon className="text-brand-primary mb-4" />
                    <h4 className="text-lg font-extrabold tracking-tight mb-2 text-brand-primary">
                      {node.title}
                    </h4>
                    <p className="text-sm font-medium text-slate-500 leading-relaxed">{node.desc}</p>
                  </div>
                );
              })}
            </div>
        ...
        <ul className="space-y-6">
          {NETWORK_BENEFITS.map((item, idx) => (
            ...
            >
              ...
              <span className="font-bold text-slate-700 tracking-tight">{item.text}</span>
            </motion.li>
          ))}
        </ul>
      </div>
