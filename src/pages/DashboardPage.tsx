...
import { TrendingUp, ArrowRight, LogOut } from 'lucide-react';
...
export const DashboardPage = () => {
  ...
  // Bump this key to force AdminGate to re-check auth after logout
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              <h1 className="text-4xl md:text-6xl font-display font-extrabold tracking-tighter text-brand-text mt-2 leading-none">
                BioDiesel<br />Dashboard
              </h1>
              <p className="text-brand-muted font-medium mt-3 max-w-md">
                Pilot phase — expanding across Egypt. Started Feb 2025.
                All financial figures are demo projections.
              </p>
            </motion.div>
            <div className="flex items-center gap-3 shrink-0">
              <motion.button
                ...
                className="h-14 px-8 bg-brand-primary text-white font-bold rounded-2xl shadow-xl shadow-brand-primary/20 hover:scale-105 transition-all flex items-center gap-2 group"
              >
                Add Partner
              ...
              <button
                ...
                className="h-14 w-14 flex items-center justify-center bg-slate-100 text-slate-500 rounded-2xl hover:bg-slate-200 hover:text-slate-900 transition-all"
              >
                <LogOut size={18} />
              </button>
            </div>
                ...
                <Card className="hover:shadow-md">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className={cn(
                      ...
                      card.positive ? 'bg-brand-secondary/10 text-brand-secondary' : 'bg-red-50 text-red-500'
                    )}>
                      {card.trend}
                    </div>
                  </div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center gap-3 bg-brand-accent/20 border border-brand-accent/40 rounded-2xl px-5 py-3">
              ...
              <p className="text-sm font-bold text-brand-primary">
                Pilot Phase Active — Financials & loyalty data below are demo projections.
                Real partner requests appear in the Quotes tab after submission.
              </p>
            </div>
          </div>
        </div>
